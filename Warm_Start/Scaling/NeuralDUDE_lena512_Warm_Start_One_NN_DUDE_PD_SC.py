...
for i in range(len(delta)):
    ...
    print 'k=1'
    C_PD,Y_PD = N_DUDE.make_data_for_One_NN_DUDE_PD(P[i],Z[i*n:(i+1)*n],1,L_new[i*alpha_size:(i+1)*alpha_size,],nb_classes,n)
    ...
    s_nn_hat=model.predict_classes(C_PD, batch_size=200, verbose=0)
    x_nn_hat=N_DUDE.denoise_with_s(z[i],s_nn_hat,1)
    error_nn=N_DUDE.error_rate(x,x_nn_hat)
        
    print '1-D N-DUDE Padding=', error_nn
        
    Error_One_NN_DUDE_PD[i,1]=error_nn
    X_hat_One_NN_DUDE_PD[k_max*i,:]=x_nn_hat
    ...
    Est_Loss_One_NN_DUDE_PD[i,1]=est_loss_nn_dude
    
    ### Save the model & weights ###
    model_json=model.to_json()
    with open("saved_model_NN_PD.json","w") as json_file:
        json_file.write(model_json)
    model.save_weights("saved_weights_NN_PD.h5",overwrite=True)
    
    for k in range(2,k_max+1):
        print 'k=',k
        ### Load the model & weights ###
        json_file=open("saved_model_NN_PD.json",'r')
        loaded_model_json=json_file.read()
        json_file.close()
        loaded_model=model_from_json(loaded_model_json)
        loaded_model.load_weights("saved_weights_NN_PD.h5")
        ...
        new_weights=np.vstack((new_rand[0:2], multiply(loaded_model.get_weights()[0],(k-1)/float(k)), new_rand[1:3])) # scaling part
        ...
        Error_One_NN_DUDE_PD[i,k]=error_nn
        X_hat_One_NN_DUDE_PD[k_max*i+k-1,:]=x_nn_hat
        
        s_class=3
        s_nn_hat_cat=np_utils.to_categorical(s_nn_hat,s_class)
        emp_dist=dot(Z[i*n:(i+1)*n,],L[i*alpha_size:(i+1)*alpha_size,])
        est_loss_nn_dude=mean(sum(emp_dist*s_nn_hat_cat,axis=1))
        Est_Loss_One_NN_DUDE_PD[i,k]=est_loss_nn_dude
        ...
        res_file='/HDD/user/yoon/Yoon_SV4/N-DUDE_SV4/NeuralDUDE_Delta_Variation/Result_Plot/Warm_Start_One_NN_DUDE_PD_Scaling'
        np.savez(res_file,Error_One_NN_DUDE_PD=Error_One_NN_DUDE_PD,
                 X_hat_One_NN_DUDE_PD=X_hat_One_NN_DUDE_PD,Est_Loss_One_NN_DUDE_PD=Est_Loss_One_NN_DUDE_PD)