...
for i in range(len(delta)):
    ...
    with open("saved_model_NN_PD_lowdelta.json","w") as json_file:
        json_file.write(model_json)
    model.save_weights("saved_weights_NN_PD_lowdelta.h5",overwrite=True)
    
    for k in range(2,k_max+1):
        print 'k=',k
        ### Load the model & weights ###
        json_file=open("saved_model_NN_PD_lowdelta.json",'r')
        loaded_model_json=json_file.read()
        json_file.close()
        loaded_model=model_from_json(loaded_model_json)
        loaded_model.load_weights("saved_weights_NN_PD_lowdelta.h5")
        ...
        res_file='/HDD/user/yoon/Yoon_SV4/N-DUDE_SV4/NeuralDUDE_Delta_Variation/Result_Plot/Warm_Start_One_NN_DUDE_PD_lowdelta'
