...
data=np.load('/home/ubuntu/Yoon_ICE/Data_Generation&Save/Neural_dude_Data_lena512_new.npz')
...
### True Loss Mat ###
Error_Two_NN_DUDE_PD_LB=zeros((len(delta),k_max+1))
Error_Two_NN_DUDE_PD = Error_Two_NN_DUDE_PD_LB.copy()
Est_Loss_Two_NN_DUDE_PD = Error_Two_NN_DUDE_PD_LB.copy()
...
### X_hat Mat ###
X_hat_Two_NN_DUDE_PD_LB=zeros((len(delta)*k_max,n))
X_hat_Two_NN_DUDE_PD = X_hat_Two_NN_DUDE_PD_LB.copy()

for j in range(1):
    i=9
    print "##### delta=%0.2f #####" % delta[i]
    for k in range(100,k_max+1):
        print 'k=',k
        
        ### 2-D N-DUDE Padding ###
        C_two_padding,Y_two_padding = N_DUDE.make_data_for_Two_rnn_n_dude_pd2(P_padding[i],P[i],k,
                                                                          L_new[i*alpha_size:(i+1)*alpha_size,],
                                                                         nb_classes,n,offset,dp)
        
        C_left = np.split(C_two_padding,2,axis=1)[0]
        C_right = np.split(C_two_padding,2,axis=1)[1]
        
        left_context = Sequential()
        left_context.add(LSTM(dim,init='he_normal',return_sequences=True,input_shape=C_left.shape[1:]))
        left_context.add(LSTM(dim,init='he_normal',return_sequences=True))
        #left_context.add(LSTM(dim,init='he_normal',return_sequences=True, go_backwards=True))
        #left_context.add(LSTM(dim,init='he_normal',return_sequences=True))
        left_context.add(LSTM(dim,init='he_normal',return_sequences=False))
        
        right_context = Sequential()
        right_context.add(LSTM(dim,init='he_normal',return_sequences=True, input_shape=C_right.shape[1:]))
        right_context.add(LSTM(dim,init='he_normal',return_sequences=True))
        #right_context.add(LSTM(dim,init='he_normal',return_sequences=True))
        #right_context.add(LSTM(dim,init='he_normal',return_sequences=True, go_backwards=True))
        right_context.add(LSTM(dim,init='he_normal',return_sequences=False))
        ...
        Hist = rnn_n_dude.fit([C_left, C_right],Y_two_padding,nb_epoch=epoch,batch_size=256,show_accuracy=True, verbose=0)
        History.append(Hist.history['loss'])
        s_nn_hat_two_PD=rnn_n_dude.predict_classes([C_left, C_right], batch_size=256, verbose=0)
        x_nn_hat_two_PD=N_DUDE.denoise_with_s_Two_NN_DUDE(z[i],s_nn_hat_two_PD)
        error_nn_two_PD=N_DUDE.error_rate(x,x_nn_hat_two_PD)
        
        print 'RNN_model_new =', error_nn_two_PD
        Error_Two_NN_DUDE_PD[i,k]=error_nn_two_PD
        X_hat_Two_NN_DUDE_PD[k_max*i+k-1,:]=x_nn_hat_two_PD
        
        s_class_two=3
        s_nn_hat_cat_two_PD=np_utils.to_categorical(s_nn_hat_two_PD,s_class_two)
        emp_dist_two_PD=dot(Z[i*n:(i+1)*n,],L[i*alpha_size:(i+1)*alpha_size,])
        est_loss_nn_dude_two_PD=mean(sum(emp_dist_two_PD*s_nn_hat_cat_two_PD,axis=1))
        Est_Loss_Two_NN_DUDE_PD[i,k]=est_loss_nn_dude_two_PD
        
        ### Save the model & weights ###
        model_json=rnn_n_dude.to_json()
        with open('./model&weights/RNN_model_new_k=%d.json'%k,"w") as model_file:
            model_file.write(model_json)
        rnn_n_dude.save_weights('./model&weights/RNN_weights_new_k=%d.h5'%k,overwrite=True)
        ...
        rnn_n_dude.compile(loss='poisson', optimizer=adam)
        rnn_n_dude.fit([C_left, C_right],Y_two_PD_lower,nb_epoch=10,batch_size=256,show_accuracy=True, verbose=0)
    
        # -----------------------------------------------------
    
        s_nn_hat_two_PD_lower=rnn_n_dude.predict_classes([C_left, C_right], batch_size=256, verbose=0)
        ...
        res_file='/home/ubuntu/Yoon_ICE/Data_Generation&Save/RNN_model_new_k=100~200'
        np.savez(res_file,Error_Two_NN_DUDE_PD = Error_Two_NN_DUDE_PD, History=History,
                 Error_Two_NN_DUDE_PD_LB=Error_Two_NN_DUDE_PD_LB,Est_Loss_Two_NN_DUDE_PD = Est_Loss_Two_NN_DUDE_PD,
                 X_hat_Two_NN_DUDE_PD = X_hat_Two_NN_DUDE_PD, X_hat_Two_NN_DUDE_PD_LB=X_hat_Two_NN_DUDE_PD_LB)